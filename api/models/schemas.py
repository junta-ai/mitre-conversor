...
class ClassificationRequest(BaseModel):
    ...
    top_k: int = Field(
        5, ge=1, le=20, description="Numero de técnicas MITRE a retornar"
    )
    use_llm: bool = Field(
        False, description="Gerar resposta amigável ao humano usando LLM"
    )
...
class ClassificationResponse(BaseModel):
    ...
    below_threshold: bool = Field(
        False,
        description="Indica se os resultados estão abaixo do threshold de similaridade",
    )
...
class SearchRequest(BaseModel):
    ...
    top_k: int = Field(
        10, ge=1, le=50, description="Numero de técnicas MITRE a retornar"
    )
