            ...
            for tactic_name, tactic_id in tactics:
                try:
                    ...
                    if "404" in page_title or "Not Found" in page_title:
                        print(f"  Página não encontrada para {tactic_name}, tentando URL alternativa...")
                    ...
                    selectors = [
                        "tbody tr td:nth-child(2) a[href*='/techniques/']"
                    ]
                    ...
                    print(f"  Encontradas {len(technique_links)} técnicas para {tactic_name}")
                    ...
                    for link in technique_links:
                        try:
                            ...
                            technique_id = technique_url.split("/techniques/")[1].split("/")[0]
                            ...
                            if not technique_url.startswith("http"):
                                technique_url = "https://attack.mitre.org" + technique_url
                            
                            print(f"  Coletando técnica: {technique_id}")
                            
                            tech_page = context.new_page()
                            try:
                                tech_page.goto(technique_url, wait_until="networkidle", timeout=30000)
                                tech_page.wait_for_selector("body", timeout=20000)
                                time.sleep(1)
                            except Exception as nav_error:
                                print(f"    Erro ao navegar para {technique_id}: {nav_error}")
                                tech_page.close()
                                continue
                            ...
                            if name_elem:
                                ...
                                if technique_id in full_name:
                                    technique_name = full_name.replace(technique_id, "").strip(" -:")
                                else:
                                    ...
                                    if paragraphs:
                                        ...
                                        for p in paragraphs[:3]:
                                            ...
                                            if (len(text) > 30 and 
                                                ...
                                                not text.startswith("Sub-techniques:") and
                                                not text.startswith("Version") and
                                                not text.startswith("Created")):
                            ...
                            if not description:
                                ...
                                for p in all_paragraphs[:10]:
                                    ...
                                    if (len(text) > 100 and 
                                        ...
                                        not text.startswith("Sub-techniques:") and
                                        not text.startswith("Version") and
                                        not text.startswith("Created") and
                                        not text.startswith("Tactics:")):
                                        description = text
                                        break
                            
                            if description and len(description) > 100 and technique_name:
                                training_entry = {
                                    "input": description,
                                    "output": {
                                        "tactic": tactic_name,
                    ...
                    techniques_collected = len([x for x in training_data if x['output']['tactic'] == tactic_name])
                    print(f"  ✓ Coletadas {techniques_collected} técnicas de {tactic_name} (de {len(technique_links)} encontradas)")
...
def save_and_display(data):
    ...
    for entry in data["training_data"]:
        tactic = entry["output"]["tactic"]
        tactic_counts[tactic] = tactic_counts.get(tactic, 0) + 1
    
    print(f"\n=== DISTRIBUIÇÃO POR TÁTICA ===")
